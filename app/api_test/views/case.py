...
from ...busines import RunCaseBusiness, CaseBusiness
from ..model_factory import ApiCasePydantic as CasePydantic, ApiProject as Project, ApiCase as Case, ApiStep as Step, \
    ApiReport as Report, ApiCaseSuite as CaseSuite
...
@api_test.login_post("/case/list", response_model=List[CasePydantic], summary="获取用例列表")
async def api_get_case_list(form: FindCaseForm, request: Request):
...
@api_test.login_put("/case/sort", summary="用例排序")
async def api_change_case_sort(form: ChangeSortForm, request: Request):
    await Case.change_sort(**form.dict(exclude_unset=True))
    return request.app.put_success()


@api_test.login_post("/case/assist/list", summary="获取造数用例集的用例list")
async def api_get_api_case_list(form: GetAssistCaseForm, request: Request):
...
@api_test.login_post("/case/name", summary="根据用例id获取用例名")
async def api_get_case_name(form: GetCaseNameForm, request: Request):
...
@api_test.login_post("/case/project", summary="获取用例属于哪个用例集、哪个用例")
async def api_get_case_project(form: GetCaseForm, request: Request):
...
@api_test.login_put("/case/status", summary="修改用例状态（是否执行）")
async def api_change_case_status(form: ChangeCaseStatusForm, request: Request):
    await Case.filter(id__in=form.id_list).update(status=form.status.value)
    return request.app.put_success()


@api_test.login_post("/case/copy", summary="复制用例")
async def api_copy_case(form: GetCaseForm, request: Request):
    case = await form.validate_request()
    new_case = await CaseBusiness.copy(case, Case, Step, request.state.user)
    return request.app.success("复制成功", data=new_case)


@api_test.login_post("/case/from", summary="获取用例的归属")
async def api_case_from(form: GetCaseForm, request: Request):
    case = await form.validate_request()
    from_path = await CaseBusiness.get_quote_case_from(case, Project, CaseSuite)
    return request.app.get_success(data=from_path)


@api_test.login_post("/case/copy/step", summary="复制指定步骤到当前用例下")
async def api_case_copy_step(form: CopyCaseStepForm, request: Request):
...
# @api_test.login_post("/case/pull/step", summary="复制指定用例的步骤到当前用例下")
# async def api_get_api_list(form: PullCaseStepForm, request: Request):
#     await form.validate_request()
#     await CaseBusiness.copy_step_to_current_case(form, Step)
#     return request.app.success("步骤复制成功，自定义变量已合并至当前用例", data=step_list)


@api_test.login_post("/case/detail", summary="获取用例详情")
async def api_get_case_detail(form: GetCaseForm, request: Request):
    case = await form.validate_request()
    return request.app.get_success(data=case)


@api_test.login_post("/case", summary="新增用例")
async def api_add_case(form: AddCaseForm, request: Request):
...
@api_test.login_put("/case", summary="修改用例")
async def api_change_case(form: EditCaseForm, request: Request):
    case = await form.validate_request()
    await case.model_update(form.dict(), request.state.user)
    return request.app.put_success()


@api_test.login_delete("/case", summary="删除用例")
async def api_delete_case(form: DeleteCaseForm, request: Request):
    case = await form.validate_request()
    await case.delete_current_and_step()
    return request.app.delete_success()


@api_test.login_post("/case/run", summary="运行测试用例")
async def api_run_case(form: RunCaseForm, request: Request):
    case_list = await form.validate_request()
