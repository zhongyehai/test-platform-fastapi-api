...
class ChangePasswordForm(BaseForm):
    ...
    async def validate_request(self, request: Request, *args, **kwargs):
        ...
        for password in [self.old_password, self.new_password, self.sure_password]:
            self.validate_is_true(3 < len(password) < password_max_length, msg=f'密码长度长度为4~{password_max_length}位')
        user = await User.filter(id=request.state.user.id).first()
        self.validate_is_true(user.verify_password(self.old_password, request.app.conf.hash_secret_key), "账号或密码错误")
...
class LoginForm(BaseForm):
    ...
    async def validate_request(self, request: Request, *args, **kwargs):
        user = await self.validate_data_is_exist("账号或密码错误", User, account=self.account)
