...
class MakeUserModel(BaseModel):
    language: str
    count: int
    options: list


@tool.post("/makeUser", summary="生成用户信息")
async def make_user_list(form: MakeUserModel, request: Request):
...
@tool.post("/makeUser/contact/download", summary="导出为通讯录文件")
async def make_contact_list(form: MakeContactModel, request: Request):
    # 数据解析为通讯录格式
    contact_text = ''
