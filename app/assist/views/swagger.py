...
from ..model_factory import SwaggerPullLog, SwaggerPullLogPydantic
from ...api_test.model_factory import ApiProject, ApiModule, ApiMsg
from ..forms.swagger import FindSwaggerPullListForm, SwaggerPullForm
...
@assist_router.login_post("/swagger/pull", summary="从swagger拉取数据")
async def pull_swagger(form: SwaggerPullForm, request: Request):
...
@assist_router.login_post("/swagger/pull/list", response_model=List[SwaggerPullLogPydantic], summary="获取拉取记录列表")
async def get_pull_swagger_log(form: FindSwaggerPullListForm, request: Request):
    query_data = await form.make_pagination(SwaggerPullLog, user=request.state.user)
