...
from ..model_factory import WebUiElement as Element, WebUiElementPydantic as ElementPydantic, WebUiProject as Project, \
    WebUiModule as Module, WebUiPage as Page
...
@ui_test.login_post("/element/list", response_model=List[ElementPydantic], summary="获取元素列表")
async def ui_get_element_list(form: ElementListForm, request: Request):
    query_data = await form.make_pagination(Element, user=request.state.user)
    return request.app.get_success(data=query_data)


@ui_test.login_put("/element/sort", summary="元素列表排序")
async def ui_change_element_sort(form: ChangeSortForm, request: Request):
    await Element.change_sort(**form.dict(exclude_unset=True))
    return request.app.put_success()


@ui_test.login_put("/element/change", summary="根据id修改元素")
async def ui_change_element_by_id(form: ChangeElementByIdForm, request: Request):
    element = await form.validate_request()
    await element.model_update(form.dict(), user=request.state.user)
    return request.app.put_success()


@ui_test.login_post("/element/template", summary="下载元素导入模板")
async def ui_get_element_template(request: Request):
    return FileResponse(os.path.join(STATIC_ADDRESS, "元素导入模板.xls"))


@ui_test.login_post("/element/upload", summary="从excel中导入元素")
async def ui_element_upload(request: Request, file: UploadFile = File(), page_id: str = Form()):
    ...
    if file and file.filename.endswith("xls"):
        ...
        if len(element_list) > 0:
            await Element.bulk_create(element_list)
        return request.app.success("元素导入成功")
    return request.app.fail("请上传后缀为xls的Excel文件")


@ui_test.login_post("/element/from", summary="获取元素的归属信息")
async def ui_get_element_from(form: GetElementForm, request: Request):
    element = await form.validate_request()
...
@ui_test.login_post("/element/detail", summary="获取元素详情")
async def ui_get_element_detail(form: GetElementForm, request: Request):
    element = await form.validate_request(request)
    return request.app.get_success(element)


@ui_test.login_post("/element", summary="新增元素")
async def ui_add_element(form: AddElementForm, request: Request):
...
@ui_test.login_put("/element", summary="修改元素")
async def ui_change_element(form: EditElementForm, request: Request):
    element = await form.validate_request(request)
    await element.model_update(form.dict(), request.state.user)
    return request.app.put_success()


@ui_test.login_delete("/element", summary="删除元素")
async def ui_delete_element(form: DeleteElementForm, request: Request):
    element = await form.validate_request(request)
